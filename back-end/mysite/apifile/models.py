...
class Player(models.Model):
    ...
    def points(time, teams):
        if(teams==None or teams ==[]):
            return Player.objects.values('user__username', 'team__name').filter(workout__workoutpoint__time__gte=time).annotate(points=Count('workout__points', default=0)).order_by('-points')
