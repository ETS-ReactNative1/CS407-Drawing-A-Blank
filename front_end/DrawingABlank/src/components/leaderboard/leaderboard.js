...
class Leaderboard extends Component {
  state = {
    ...
    default_pictures: {
      ocean: require('../../assets/img/ocean.png'),
  ...
  getLeaderboards = () => {
    dateArgument =
      this.state.dateChosen != ''
        ? this.state.dateChosen.toLocaleString('en-GB').split(',')[0]
        : '01/01/1970';
    teamArgument = this.state.selectedOptions;
    console.log(
      'SENDING REQUEST WITH DATE ' + dateArgument + ' TEAMS ' + teamArgument,
    );
    getPointsLeaderboard(dateArgument, teamArgument)
      .then(points_res => {
        console.log(points_res);
        this.setState({leaderboard_points: points_res}, () => {
          this.getPlayerCardPoints();
        });
      })
      .then(() => {
        getDistanceLeaderboard(dateArgument, teamArgument).then(
          distance_res => {
            this.setState({leaderboard_distance: distance_res}, () => {
              this.getPlayerCardDistance();
            });
          },
        );
      });
  ...
  getPlayerCardPoints = () => {
    getUsername().then(username => {
      ...
      var details = this.state.leaderboard_points.find(
        user => user.name == username,
      );
      var index = this.state.leaderboard_points.findIndex(
        user => user.name == username,
      );
  ...
  getPlayerCardDistance = () => {
    getUsername().then(username => {
      console.log('OBTAINED USERNAME:' + username);
      var details = this.state.leaderboard_distance.find(
        user => user.name == username,
      );
      var index = this.state.leaderboard_distance.findIndex(
        user => user.name == username,
      );
      if (details != undefined) {
        this.setState(
  ...
  getPlayerCard = () => {
    if (
      ...
      Object.keys(this.state.player_card_points).length == 0
    ) {
      return <View />;
    }
    ...
    return (
      <View style={styles.leaderboard}>
        ...
        <ScrollView
          ...
          <View style={styles.leaderboard_entry}>
            ...
            <View style={styles.leaderboard_entry_score}>
              <Text style={styles.leaderboard_entry_score_text}>
                {this.state.points_selected ? 'Score' : 'Distance'}
              </Text>
            </View>
          </View>
          ...
          {this.state.collectedLeaderboards &&
            (this.state.points_selected
              ? this.state.leaderboard_points.map((info, index) => {
                  {
                    if (info.name.includes(this.state.username_search)) {
                      return (
                        <TouchableOpacity
                          ...
                          <View style={styles.leaderboard_entry_team}>
                            <Text style={styles.leaderboard_entry_team_text}>
                              {this.state.api_to_label_teams[info.team]}
                            </Text>
                          </View>
                          <View style={styles.leaderboard_entry_score}>
                            <Text style={styles.leaderboard_entry_score_text}>
