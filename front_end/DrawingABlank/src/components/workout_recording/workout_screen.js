...
class WorkoutScreen extends Component {
  ...
  componentDidMount() {
    ...
    setInterval(() => {
      Geolocation.getCurrentPosition(
        ({coords}) => this.updateDebugText(coords),
        error => console.log(error),
        {enableHighAccuracy: true, timeout: 20000, maximumAge: 1000},
      );
    }, 1000);
  ...
  getDetailText() {
    ...
    for (var c = 0; c < recorder.coordinates.length; c++) {
      result +=
        '(' +
        recorder.coordinates[c].latitude +
        ',' +
        recorder.coordinates[c].longitude +
        ') at ' +
        recorder.coordinates[c].timestamp +
        '\n';
    }
    result += '\nAverage speed:' + recorder.getAverageSpeed() + 'm/s\n';
    result += 'Total distance:' + recorder.getTotalDistance() + 'm\n';
    result +=
      'Speed vs Time JSON:' + JSON.stringify(recorder.getSpeedvsTime()) + '\n';
    result +=
      'Distance vs Time JSON:' +
      JSON.stringify(recorder.getDistanceVsTime()) +
      '\n';
    return result;
  }
  getExtraData() {
